...
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, PieChart, Pie, Cell, Legend, BarChart, Bar, LineChart, Line } from 'recharts';
...
const Analytics: React.FC = () => {
    const [weeklyData, setWeeklyData] = useState<{ date: string, hours: number, confidence?: number }[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // 2. Trend Data with Confidence Correlation
    ...
    const COLORS = ['#8b5cf6', '#0ea5e9', '#22c55e', '#f43f5e', '#f59e0b', '#ec4899'];
        ...
        <div className="space-y-8 pb-12">
            ...
            <div className="bg-gradient-to-br from-indigo-900 via-brand-900 to-purple-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    ...
                    <div className="bg-white/10 backdrop-blur-xl p-6 rounded-2xl border border-white/20 shadow-2xl min-w-[280px]">
                        ...
                        <div className="text-center mt-4 text-xs font-medium text-indigo-100">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-700">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">Time invested vs. Self-rated confidence</p>
                        </div>
                    ...
                    <div className="h-80 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={weeklyData}>
                                ...
                                <YAxis yAxisId="left" stroke="#9ca3af" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(v) => `${v}h`} />
                                <YAxis yAxisId="right" orientation="right" stroke="#9ca3af" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(v) => `${v}%`} />
                                <Tooltip
                                    contentStyle={{ backgroundColor: '#1f2937', color: '#fff', border: 'none', borderRadius: '16px', padding: '12px' }}
                                    itemStyle={{ color: '#fff' }}
                                />
                                <Area yAxisId="left" type="monotone" dataKey="hours" stroke="#8b5cf6" strokeWidth={4} fill="url(#colorMins)" />
                                <Line yAxisId="right" type="monotone" dataKey="confidence" stroke="#f59e0b" strokeWidth={3} dot={{ fill: '#f59e0b', r: 4 }} activeDot={{ r: 6 }} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                    <div className="mt-6 flex justify-center gap-6 text-xs font-bold uppercase tracking-wider">
                        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-purple-500 rounded-full"></div> Hours Studied</div>
                        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-amber-500 rounded-full"></div> Confidence Level</div>
                    </div>
                </div>

                {/* Subject Mastery Pie */}
                <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-700">
                    <h2 className="text-2xl font-black mb-2 dark:text-white">Prep Balance</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-8">How you are distributing your focus</p>
                    <div className="h-80 w-full">
                        {subjectData.length > 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-gray-400">
                                ...
                                <span className="font-medium">Start your first session to see analysis!</span>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                <div className="p-8 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/20">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Detailed breakdown of your growth journey</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[700px]">
                        <thead>
                            <tr className="bg-gray-50/50 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">
                                ...
                                <th className="px-8 py-5 text-right w-32">Total Effort</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    <td colSpan={3} className="px-8 py-20 text-center text-gray-400 font-medium">
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <span>No history yet. Your future self is waiting for you to start!</span>
                                        </div>
                                    </td>
                                </tr>
                            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div className="text-xl font-black text-gray-900 dark:text-white">Strong</div>
                    </div>
                </div>
            </div>
        </div>
