...
const Analytics: React.FC = () => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            const allConf = await api.getConfidence(currentUser.uid);
            if (allConf.length > 0) {
            ...
            const history = await api.getConfidenceHistory(currentUser.uid);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 flex flex-col justify-center items-center text-center">
                    <div className="text-gray-500 dark:text-gray-400 text-sm mb-2">Average Confidence</div>
                    <div className="text-3xl font-bold text-gray-900 dark:text-white">
                        {avgConfidence !== null ? `${avgConfidence}%` : '-'}
                    </div>
                    <div className="text-xs text-indigo-500 mt-1">Self-rated performance</div>
                </div>
            </div>
            ...
            {confidenceHistory.length > 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
                    <h3 className="text-lg font-bold mb-4 dark:text-white">Confidence Meter History</h3>
                    <div className="space-y-3">
                        {confidenceHistory.map((entry, idx) => (
